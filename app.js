const API="/api/proxy";
const API_KEY_NOT_IN_BROWSER=true;
...
function buildUrl(t){
 ...
 p.set("tool",t.endpoint.replace(".php",""));
...
async function runTool(id){
 ...
 box.innerHTML=`<div class="loading"><span class="spinner"></span> Menghubungkan ke PIDZ API...</div>`;
 try{
   ...
   if(!res.ok){
     ...
     const upstream=res.headers.get("X-PIDZ-Upstream-Status");
     box.innerHTML=`<pre>Request gagal: HTTP ${res.status}${upstream?` (upstream HTTP ${upstream})`:""}\n\n${escapeHtml(detail)}</pre>`;
     return;
   }
   if(type.includes("application/json") || type.includes("text/json")){
     ...
     const tag=type.startsWith("image/")?"img":type.startsWith("video/")?"video":"audio";
     box.innerHTML=`<${tag} src="${url}" ${tag!=="img"?"controls":""} style="max-width:100%;width:100%;border-radius:12px"></${tag}>`;
   }else{
     const text=await res.text();
     box.innerHTML=`<pre>${escapeHtml(text)}</pre>`;
   }
 }catch(e){
   box.innerHTML=`<pre>Request gagal: ${escapeHtml(e.message)}

Jika error berisi "whois.nic.google", itu berasal dari server API upstream, bukan dari UI PIDZ TOOLS. Endpoint upstream sedang gagal melakukan DNS/WHOIS lookup.</pre>`;
 }
}
function collectUrls(obj, path=""){
 ...
 if(typeof obj==="string"){
   ...
   for(const u of matches) out.push({url:u.replace(/[),.]+$/,""),label:path||"Link"});
 }else if(Array.isArray(obj)){
   obj.forEach((v,i)=>out.push(...collectUrls(v,path+"["+i+"]")));
 }else if(obj && typeof obj==="object"){
   for(const [k,v] of Object.entries(obj)){
     const p=path?path+"."+k:k;
     if(typeof v==="string" && /^https?:\/\//i.test(v)) out.push({url:v,label:k});
     else out.push(...collectUrls(v,p));
   }
 }
 return out;
}
function renderResult(data,box){
 ...
 let html="";
 if(unique.length){
   html+=`<div style="display:grid;gap:9px;margin-bottom:15px">
   ${unique.slice(0,12).map((x,i)=>`<a href="${safeUrl(x.url)}" target="_blank" rel="noopener noreferrer" class="btn" style="display:block;text-align:center;text-decoration:none">${downloadLabel(x.label,i)}</a>`).join("")}
   </div>`;
 }
 html+=`<pre>${escapeHtml(JSON.stringify(data,null,2))}</pre>`;
 box.innerHTML=html;
}
function downloadLabel(label,i){
 const l=String(label).toLowerCase();
 if(l.includes("audio")||l.includes("mp3"))return "♫ BUKA AUDIO";
 if(l.includes("video")||l.includes("mp4")||l.includes("download")||l.includes("url"))return "⇩ DOWNLOAD / OPEN HASIL";
 return `OPEN RESULT ${i+1}`;
}
